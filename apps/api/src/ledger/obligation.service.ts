  ...
  async recordContributionPayment(
    ...
    return this.prisma.withTenant(actor.organisationId, async (tx) => {
      ...
      if (organisation.paymentAllocationPolicy !== 'oldest_first') {
        ...
        // only the one every source constitution's "arrears before
        // current dues" pattern actually needed is implemented.
      ...
      if (remaining.greaterThan(0)) {
        ...
        // creates or credits one yet). Reject rather than silently
        // dropping the excess.
