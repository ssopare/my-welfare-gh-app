...
### Subscription billing (post-Phase-1, §18)

The last item in the post-Phase-1 track. Phase 1 scope per the roadmap table: free trial + one flat paid tier, operator-editable pricing table — the full Starter/Growth/Enterprise matrix and Federation-tier bundling (FR-SUB-06) are Phase 2, once more than one paid tier exists. See `src/subscriptions/`, `src/platform-auth/`.

- **FR-SUB-01, enforced structurally, not by convention:** `Subscription`/`SubscriptionPlan` have no relation anywhere to `Fund`/`JournalEntry`/`LedgerAccount`/`Obligation` — platform revenue can never be confused with, or touch, a tenant's own welfare funds.
- **A wholly new kind of actor: `PlatformOperator`.** Not a `Member` of any tenant — one level further out than `Account` itself (see that model's "deliberately NOT tenant-scoped" comment). Its own JWT type (`{ sub, type: 'platform_operator' }`, `PlatformAuthGuard`), signed with the same `JWT_SECRET` as tenant tokens purely for operational simplicity — the `type` discriminator is what stops the two from ever being mistaken for each other even though both verify against the same secret. No self-registration endpoint: provisioned by `scripts/create-platform-operator.ts` (`npm run platform:create-operator -- <email> <password>`), an ops action, not a product surface — there's no "first admin" bootstrap problem to solve here.
- **`GET /subscription-plans` is public** (a pricing page needs it before any account exists); creating/archiving plans (`POST`/`PATCH /platform/subscription-plans/...`, FR-SUB-04) is `PlatformAuthGuard`-only.
- **Every new `Organisation` gets a `TRIAL` subscription automatically** (§18.1), created inside `AuthService.registerOrganisation`'s own transaction so a failure rolls back the whole registration rather than leaving a tenant with no subscription row. `planId` stays null until the tenant actually converts (`POST /subscription/convert`) — a trial predates plan selection, so `trialEndsAt` comes from a platform-wide default, not a plan that might not exist yet.
- **`SubscriptionGuard` (global, via `APP_GUARD`) blocks new writes — never reads or exports — on a `SUSPENDED`/`CANCELLED`/lapsed-trial subscription**, returning `402 Payment Required`. "Global" here specifically means it runs *before* any controller's `@UseGuards(JwtAuthGuard)`, so it can't rely on that guard having already decoded the token — it decodes independently (same `JwtService`) and only ever acts when it positively recognises a *tenant* payload (has `organisationId`); anything else — no token, an invalid one, or a platform operator's — is a no-op, real authentication is still entirely `JwtAuthGuard`/`PlatformAuthGuard`'s job. `@SkipSubscriptionCheck()` exempts the one endpoint a suspended tenant must still reach: `POST /subscription/convert`, its own way out.
- **The platform operator's manual lever** (`PATCH /platform/organisations/:id/subscription`, `GET /platform/subscriptions` — both cross-tenant via `PrismaService.withPlatformOperatorContext()`, a second independent permissive RLS policy on `subscriptions` alongside its ordinary `tenant_isolation` one, same "OR'd policies" shape as the scheduler's) is Phase 1's stand-in for real payment-gateway/dunning automation — the same bootstrapping every other slice started with (`MockPaymentProvider` stood in for a live gateway before `FR-PAY-01` existed; manual status changes stand in here).
- **The daily sweep formally suspends a lapsed trial** and sends a `SUBSCRIPTION_LAPSED` notification to the org's admins — `SubscriptionGuard` already blocks writes the instant a trial's `trialEndsAt` passes (computed at request time), so this is about making the *stored* status catch up to reality for anything that reads it directly, like the operator's own dashboard, not the enforcement itself.
